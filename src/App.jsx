...
import { fmt, tod, isToday, isOver, appendNoteToMd, buildTrackerRow, appendTrackerRow, buildMeetingMd } from './utils/formatter.js';
...
export default function App() {
  ...
  const resumeFolder = async () => {
    ...
    try {
      ...
      if (perm==='granted') { setDirHandle(savedHandle); setFolder(savedHandle.name); setSavedHandle(null); await loadFiles(savedHandle); }
    } catch(e) { console.error(e); }
    setLoading(false);
  };
  ...
  const saveMeetingFile = useCallback(async () => {
    if (!dirHandle || !meetingStartRef.current) return;
  ...
  const filtered  = tasks.filter(t => filt==='today'?isToday(t.due):filt==='overdue'?isOver(t.due):filt==='done'?t.status==='done':true);
  ...
  if (!folder) return (
    <div style={{ height:'100vh', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:20, color:'#e2e8f0', background:'radial-gradient(ellipse at 50% -5%,rgba(124,58,237,0.22) 0%,#09090e 65%)' }}>
      ...
      {savedHandle ? (
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:10 }}>
          <button onClick={resumeFolder} disabled={loading} style={btnPrimary}>{loading?'Loading…':`▶  Resume — ${savedHandle.name}`}</button>
          <button onClick={pickFolder} style={{ background:'none', border:'none', color:'#475569', cursor:'pointer', fontSize:12, fontFamily:'inherit' }}>or select a different folder</button>
        </div>
      ) : (
        <button onClick={pickFolder} disabled={loading} style={btnPrimary}>{loading?'Reading files…':'📁  Select Vault Folder'}</button>
      )}
        ...
        <div style={{ padding:'18px 14px 12px', borderBottom:'1px solid rgba(255,255,255,0.06)' }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:4 }}>
            <div style={{ display:'flex', alignItems:'center', gap:7 }}>
              ...
              <span style={{ fontWeight:700, fontSize:13, maxWidth:155, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{folder}</span>
            </div>
            <button onClick={() => loadFiles(dirHandle)} style={{ padding:'4px 10px', borderRadius:7, border:'none', cursor:'pointer', fontSize:11, fontWeight:600, fontFamily:'inherit',
